...
import { deleteDepartmentApi } from "../../../Api/DepartmentMasterApi";
import { Edit, Delete } from "@material-ui/icons";
import { deleteHostApi, getAllHostApi } from "../../../Api/HostMasterApi";
...
const HostMaster = () => {
    ...
    const [allHost, setAllHost] = useState([]); // Dummy empty state
    ...
    const getHostData = (hostId) => {
        navigate('/addHostMaster', { state: { hostId } });
    };

    const DeleteHostData = async (hostId) => {
        const data = await deleteHostApi(hostId, navigate);
        console.log(data)
        getAllHost()
    }
    ...
    const handleSearch = (e) => {
        ...
        if (searchDataValue.trim() === "") {
            ...
            const filteredData = allHost.filter(
                (department) =>
                    department.HostName.toLowerCase().includes(searchDataValue) ||
                    department.Email.toLowerCase().includes(searchDataValue) ||
                    department.MobileNumber.toLowerCase().includes(searchData)
            );
            setAllHost(filteredData);
            setCurrentPage(1);
        }
    };
    ...
    const currentItems = allHost.slice(indexOfFirstItem, indexOfLastItem);
                                        ...
                                        <div className="row">
                                            ...
                                            <div className="col-lg-2 d-flex justify-content-end">
                                                ...
                                                <button
                                                    ...
                                                    onClick={() => navigate("/addHostMaster")}
                                                >
                                                    Add
                                                </button>
                                            </div>
                                                        ...
                                                        <td>
                                                            <div className="d-flex ">
                                                                <Edit
                                                                    ...
                                                                    type="button"
                                                                    onClick={() => getHostData(data.Id)}

                                                                />
                                                                <Delete
                                                                    className="text-danger"
                                                                    type="button"
                                                                    onClick={() => DeleteHostData(data.Id)}
                                        ...
                                        <div className="row mt-4 mt-xl-3">
                                            <div className="col-lg-4 col-md-4 col-12">
                                                <h6 className="text-lg-start text-center"> Showing {indexOfFirstItem + 1} to{" "}
                                                    {Math.min(indexOfLastItem, allHost.length)} of{" "}
                                                    {allHost.length} entries</h6>
                                            </div>
